...
#%%
datum    =  datetime.datetime.strptime( str( sys.argv[1]) ,'%Y-%m-%d') #datetime.datetime.strptime(str( sys.argv[1]  ,'%Y-%m-%d')
forecast =  str(sys.argv[2])  #'00'  '12' 
...
def plotter(timestep, vmin, vmax, pol, pol_name_in_post,pol_name_save, plt_save_path, save_plot = False ):
    ...
    plt.title(f"forecast from {forecast}UTC {datum_strf}, to + {timefrombegin} UTC ")
    if save_plot == True:
